...
# Copy the kit over the demo root, but never clobber the repo's own files
# (README / AGENTS.md / .git / .nojekyll / publish.sh).
rsync -a --exclude README.md --exclude AGENTS.md --exclude .git --exclude .nojekyll --exclude publish.sh "$SRC"/ "$HERE"/
git -C "$HERE" add -A
